...
public class SimpleCollectionNumbers<T> {
    int[] numbers;
    ...
    public int indexOf(int value) throws EmptyCollection {
        int size = size();

        if (size == 0) {
            throw new EmptyCollection("Collection has no elements. The size of the collection - ", size());
        }
    ...
    public int get(int index) throws NullElement {
        try {
            return numbers[index];
        } catch (ArrayIndexOutOfBoundsException e) {
            throw new NullElement("Element is NULL!");
        }
    }
